...
import {
  ...
  TouchableOpacity, ActivityIndicator, Image, Alert, Modal
} from 'react-native';
...
export default function LibraryScreen() {
  ...
  const fetchLibrary = async () => {
    ...
    try {
      ...
      setLibraryData(data); // { games: [...], assets: [...] }
    } catch (error) {
  ...
  const pickAndUploadMedia = async (mediaType) => {
    ...
    if (!result.canceled) {
      ...
      formData.append('mediaFile', { uri: localUri, name: filename, type });
      ...
      try {
        ...
        if (data.status === 'Success') {
          Alert.alert('Başarılı!', 'Test veriniz geliştiriciye iletildi. Katkınız için teşekkürler.');
          ...
          <View style={styles.modalContent}>
            ...
            {isUploading ? (
              <View style={{ padding: 40, alignItems: 'center' }}>
                ...
                <Text style={{ color: COLORS.white, marginTop: 15 }}>Medya Yükleniyor... Lütfen bekleyin.</Text>
              </View>
            ) : (
              <View style={styles.modalButtons}>
                <TouchableOpacity style={[styles.uploadActionBtn, { backgroundColor: '#e94560' }]} onPress={() => pickAndUploadMedia('video')}>
                  <Text style={{ fontSize: 24, marginBottom: 5 }}>🎥</Text>
                  <Text style={styles.uploadActionText}>Ekran Kaydı Seç</Text>
                </TouchableOpacity>

                <TouchableOpacity style={[styles.uploadActionBtn, { backgroundColor: '#5b5bfe' }]} onPress={() => pickAndUploadMedia('image')}>
                  <Text style={{ fontSize: 24, marginBottom: 5 }}>🖼️</Text>
                  <Text style={styles.uploadActionText}>Ekran Görüntüsü Seç</Text>
                </TouchableOpacity>
              </View>
            )}

            <TouchableOpacity onPress={() => !isUploading && setTestModalVisible(false)} style={styles.closeBtn}>
